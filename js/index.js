...
function displayMeals(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartoona += `
        <div class="col-md-3 ">
                <div onclick="getMealDetails('${arr[i].idMeal}')" class="meal position-relative 
                   overflow-hidden rounded-2 cursor-pointer product">
...
function displayCategory(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartoona += `
        <div class="col-md-3">
                <div onclick="getCategoryMeals('${arr[i].strCategory}')" class="meal position-relative 
                  overflow-hidden rounded-2 cursor-pointer product">
                    <img class="w-100" src="${arr[i].strCategoryThumb}" alt="meal">
                    <div class="meal-layer position-absolute text-center text-black p-2">
                        <h3 >${arr[i].strCategory}</h3>
                        <p class="text-muted">${arr[i].strCategoryDescription.split(" ").slice(0, 20).join(" ")}</p>
                    </div>
                </div>
            </div>
...
function displayArea(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartona +=
            `<div class="col-md-3 ">
        <div onclick="getAreaMeals('${arr[i].strArea}')" class="text-center rounded-2 bg-white  p-2 my-2
         cursor-pointer product">
            <i class="fa-solid fa-city fa-3x  mb-2 text-warning "></i>
            <h2 class="text-muted fw-light">${arr[i].strArea}</h2>
        </div>
    </div>
        `
    }
...
function displayIngredients(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartona += `<div class="col-md-3 ">
                     ...
                     class=" rounded-2  bg-white text-center py-4 my-2 cursor-pointer product">
                       <i class="fa-solid fa-bowl-food fa-4x text-warning"></i>
                       <h3 class="text-black fw-light">${arr[i].strIngredient}</h3>
...
function displayMealDetails(meal) {
    ...
    for (let i = 1; i <= 20; i++) {
        if (meal[`strIngredient${i}`]) {
            ingredients += `<li class="alert alert-success m-1 p-1">${meal[`strMeasure${i}`]} 
    ...
    <img class="w-100 rounded-3 product" src="${meal.strMealThumb}" alt="meal">
    <h2 class="py-2 text-warning">${meal.strMeal}</h2>
    </div>
    <div class="col-md-8">
    <h2 class="fw-light py-2 text-warning">Instructions</h2>
    <p class="text-muted ">${meal.strInstructions}</p>
    <h3 ><span class=" text-warning fw-light">Area : </span>${meal.strArea}</h3>
    <h3 ><span class="fw-light text-warning">Category : </span>${meal.strCategory}</h3>
    <h3 ><span class="fw-light text-warning">Recipes : </span>${meal.strRecipes}</h3>
    <ul class="list-unstyled d-flex g-3 flex-wrap">
        ${ingredients}
    </ul>
    <h3 class="fw-light text-warning">Tags :</h3>
    <ul class="list-unstyled d-flex g-3 flex-wrap">
       ${tagsStr}
    </ul>
